...
public class CSVLoaderControl {
    private static int studentCount = 0;
    private static int staffCount = 0;

    public static StudentEntity[] loadStudents() {
        String[][] data = CSVReaderControl.readCSV("sample_student_list.csv", 100, 10);
        ...
        for (int i = 0; i < data.length && data[i][0] != null; i++) {
            ...
            students[studentCount++] = new StudentEntity(id, name, major, year, email);
        }
    ...
    public static CareerCentreStaffEntity[] loadStaff() {
        String[][] data = CSVReaderControl.readCSV("sample_staff_list.csv", 50, 10);
        CareerCentreStaffEntity[] staff = new CareerCentreStaffEntity[50];
        staffCount = 0;
        ...
        for (int i = 0; i < data.length && data[i][0] != null; i++) {
            ...
            staff[staffCount++] = new CareerCentreStaffEntity(id, name, department, email);
        }
    ...
    public static CompanyRepEntity[] loadCompanyReps() {
        String[][] data = CSVReaderControl.readCSV("sample_company_representative_list.csv", 50, 10);
        CompanyRepEntity[] reps = new CompanyRepEntity[50];
        companyRepCount = 0;
        ...
        for (int i = 0; i < data.length && data[i][0] != null; i++) {
            ...
            CompanyRepEntity rep = new CompanyRepEntity(id, name, companyName, department, position, email);
    ...
    public static InternshipEntity[] loadInternships() {
        String[][] data = CSVReaderControl.readCSV("sample_internship_list.csv", 100, 15);
        InternshipEntity[] internships = new InternshipEntity[100];
        internshipCount = 0;
        ...
        for (int i = 0; i < data.length && data[i][0] != null; i++) {
            ...
            if (!InputValidatorControl.isValidMajor(preferredMajor)) {
                System.out.println("WARNING: Invalid major '" + preferredMajor + "' for internship '" + title + "'");
                hasWarning = true;
            } else {
                preferredMajor = InputValidatorControl.normalizeMajor(preferredMajor);
            }
            ...
            if (!InputValidatorControl.isValidDate(closingDate)) {
                ...
                hasWarning = true;
            } else if (InputValidatorControl.isValidDate(openingDate) && 
            ...
            InternshipEntity internship = new InternshipEntity(title, description, level, preferredMajor, 
